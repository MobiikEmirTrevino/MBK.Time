...
namespace MBK.Time.Web.Mvc.Controllers
{
    ...
    public class DashboardController : SFSdotNet.Framework.Web.Mvc.ControllerBase
    {
        ...
        {
            if (SFSdotNet.Framework.Cache.Caching.ExistInCompany("dashboard-tota-approved", "MBKTime", SFSdotNet.Framework.My.Context.CurrentContext.Company.GuidCompany))
            {
                return (DataGraph)SFSdotNet.Framework.Cache.Caching.GetFromCompany("dashboard-tota-approved", "MBKTime", SFSdotNet.Framework.My.Context.CurrentContext.Company.GuidCompany);
            }
            else
            {
                var dataGraph = new DataGraph();
                dataGraph.Title = "Aprobadas";
               // dataGraph.Total = MBK.Time.BR.accIncomeExpensesBR.Instance.GetCount(p => p.accReusableCatalogValue.ValueString == "money-out-request" && p.Authorized == true);

                SFSdotNet.Framework.Cache.Caching.SetToCompany(dataGraph, "dashboard-tota-approved", "MBKTime", SFSdotNet.Framework.My.Context.CurrentContext.Company.GuidCompany, TimeSpan.FromMinutes(60));
                return dataGraph;
        ...
        {
            if (SFSdotNet.Framework.Cache.Caching.ExistInCompany("dashboard-tota-inprocess", "MBKTime", SFSdotNet.Framework.My.Context.CurrentContext.Company.GuidCompany))
            {
                return (DataGraph<int>)SFSdotNet.Framework.Cache.Caching.GetFromCompany("dashboard-tota-inprocess", "MBKTime", SFSdotNet.Framework.My.Context.CurrentContext.Company.GuidCompany);
            }
            else
            {
                var dataGraph = new DataGraph<int>();
                dataGraph.Title = "En proceso";
                //dataGraph.Total = MBK.Time.BR.accIncomeExpensesBR.Instance.GetCount(p => p.accReusableCatalogValue.ValueString == "money-out-request" && p.Authorized == null);

                SFSdotNet.Framework.Cache.Caching.SetToCompany(dataGraph, "dashboard-tota-inprocess", "MBKTime", SFSdotNet.Framework.My.Context.CurrentContext.Company.GuidCompany, TimeSpan.FromMinutes(60));
                return dataGraph;
        ...
        {
            if (SFSdotNet.Framework.Cache.Caching.ExistInCompany("dashboard-total-rejected", "MBKTime", SFSdotNet.Framework.My.Context.CurrentContext.Company.GuidCompany))
            {
                return (DataGraph)SFSdotNet.Framework.Cache.Caching.GetFromCompany("dashboard-total-rejected", "MBKTime", SFSdotNet.Framework.My.Context.CurrentContext.Company.GuidCompany);
            }
            else
            {
                var dataGraph = new DataGraph();
                dataGraph.Title = "Rechazadas";
              //  dataGraph.Total = MBK.Time.BR.accIncomeExpensesBR.Instance.GetCount(p => p.accReusableCatalogValue.ValueString == "money-out-request" && p.Authorized == false);

                SFSdotNet.Framework.Cache.Caching.SetToCompany(dataGraph, "dashboard-total-rejected", "MBKTime", SFSdotNet.Framework.My.Context.CurrentContext.Company.GuidCompany, TimeSpan.FromMinutes(60));
                return dataGraph;
        ...
        {
            if (SFSdotNet.Framework.Cache.Caching.ExistInCompany("dashboard-total-payment", "MBKTime", SFSdotNet.Framework.My.Context.CurrentContext.Company.GuidCompany))
            {
                return (DataGraph)SFSdotNet.Framework.Cache.Caching.GetFromCompany("dashboard-total-payment", "MBKTime", SFSdotNet.Framework.My.Context.CurrentContext.Company.GuidCompany);
            }
            else
            {
                var dataGraph = new DataGraph<int>();
                dataGraph.Title = "Pagadas";
               // dataGraph.Total = MBK.Time.BR.accIncomeExpensesBR.Instance.GetCount(p => p.accReusableCatalogValue.ValueString == "money-out-request"  && p.TotalPayment >= p.TotalChilds);

                SFSdotNet.Framework.Cache.Caching.SetToCompany(dataGraph, "dashboard-total-payment", "MBKTime", SFSdotNet.Framework.My.Context.CurrentContext.Company.GuidCompany, TimeSpan.FromMinutes(60));
                return dataGraph;
        ...
        public ActionResult Index() {
            ...
            if (ViewData["uiv"] == null)
            {
                ...
                //if ((new SFSdotNet.Framework.Security.Permissions()).IsAllowed(SFSdotNet.Framework.My.Context.CurrentContext.User, "MBKTime", accIncomeExpens.EntityName + "_money-in", "r"))
                //{
                //    list = new DashboardItemList();
                //    list.UrlView = VirtualPathUtility.ToAbsolute("~/") + "MBKTime/accIncomeExpenses/ListViewGen?usemode=money-in";
                //    list.Title = "Ingresos";
                //    list.UrlCatalog = VirtualPathUtility.ToAbsolute("~/") + "MBKTime/accIncomeExpenses?usemode=money-in";

                //    lists.Add(list);
                //}
                //if ((new SFSdotNet.Framework.Security.Permissions()).IsAllowed(SFSdotNet.Framework.My.Context.CurrentContext.User, "MBKTime", accIncomeExpens.EntityName + "_money-out-request", "r"))
                //{
                //    list = new DashboardItemList();
                //    list.UrlView = VirtualPathUtility.ToAbsolute("~/") + "MBKTime/accIncomeExpenses/ListViewGen?usemode=money-out-request";
                //    list.Title = "Solicitudes de egreso";
                //    list.UrlCatalog = VirtualPathUtility.ToAbsolute("~/") + "MBKTime/accIncomeExpenses?usemode=money-out-request";
        ...
        public DataGraph<int> GetHistoryDataDefitition()
        {
            ...
            dataGraph.UrlData = VirtualPathUtility.ToAbsolute("~/") + "MBKTime/Dashboard/GetDataGraphJson/HistoricalData";
            return dataGraph;
        }
